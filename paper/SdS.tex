...
We test a fixed-background prerequisite for this proposal with
curvature-coupled scalar waves on Schwarzschild--de Sitter (SdS) spacetime;
the calculation does not test the regularity or conditioning of the conformal
Einstein equations.  We formulate minimal coupling, $\xi=0$, and conformal
coupling, $\xi=1/6$.  The former retains a curvature-dependent lower-order
term after spacetime compactification, whereas the latter is the scalar
analogue of Misner's conformally invariant Maxwell test.
A controlled large-$L$ comparison requires the metric, foliation,
compactification, initial-data map, and waveform clock to approach the same
Schwarzschild problem.  We construct such a family with a bridge foliation
and compare it with a separately evolved Schwarzschild reference.  The broad
waveform and caustic suite uses $\xi=0$.  For
$L/M=80,160,320,640$, the direct waveform error decreases, although its size
depends on the observable.  Two nested extrapolants support recovery of the
Schwarzschild waveform within a conservative $1\%$ for a cumulative,
prompt-dominated
pure-$\ell=2$ test through $U/M=160$ and for a localized multimode $3+1$
response on the common interval $0\lesssim U/M\leq57.2$.  This bound is not
uniform at late times.  Caustic-echo timing is consistent with local
$\mathcal O((M/L)^2)$ and outer $\mathcal O(M/L)$ trends, but deterministic
sensitivities preclude precise fit coefficients.
We also construct a static, spherically symmetric exterior-supported family
whose conserved cosmological tensor vanishes in the strong-field region and
reaches the constant positive-$\Lambda$ form beyond a smooth transition.  At
fixed exterior $\Lambda$, this construction improves the raw, unshifted
pure-$\ell=2$ quasinormal-mode (QNM) window agreement with Schwarzschild.
Across three fixed $30M$ windows, the relative error reduction ranges from
$20.9$--$30.2\%$ at $L/M=80$ to $7.8$--$9.3\%$ at $L/M=640$.  The observed
fine-grid ordering clears conservative level-to-level margins at every $L$,
although the $L/M=640$ exterior ladder is nonmonotone and is not continuum
extrapolated.  The improvement remains modest at large $L$ and does not
determine the global quasinormal spectrum.
A matched $\ell=1$ outer-boundary study through $U/M=1000$ compares both
...
secondary lobes produced by the altered scattering problem.  Neither uniform
case resolves its expected asymptotic cosmological rate on the evolved
interval, and no asymptotic rate is assigned to the exterior family.  The
results support artificial cosmology as a finite-time waveform regulator but
show that tail observables remain sensitive to the asymptotic structure.
Application to the nonlinear Einstein equations is the next test.
\end{abstract}
