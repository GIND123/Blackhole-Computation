...
response representing an angle-dependent $3+1$ field on its common simulated
interval.  We make no claim of uniform
...
ZenginogluBernuzziNutzi:2026}.  The present calculation complements those
tests with an angle-dependent scalar field on black-hole backgrounds: the
outer boundary is the cosmological horizon at finite $L$ and $\scri^+$ for
the separately evolved Schwarzschild reference.  Such multidimensional tests
expose issues of angular resolution, spherical boundaries, and long-time
evolution that a $1+1$ reduction cannot reveal.  They are important
preparation for a nonlinear Einstein implementation.
...
This is a $3+1$ field calculation implemented by harmonic reduction: the
spherical background decouples the radial evolution by $\ell$, while the
source and reconstructed field retain their dependence on
$(r,\theta,\varphi)$.  Thus the calculation keeps the full angular dependence
without using a grid-based angular discretization.  Its outer endpoint is
$\hc$ for every finite-$L$ run and $\scri^+$ for the separately evolved
...
unresolved disjoint late-time windows.  The localized calculation also gives
an angle-dependent $3+1$ black-hole field benchmark for the outer-boundary
geometry, although the spherical background permits a reduction to radial
modes.
