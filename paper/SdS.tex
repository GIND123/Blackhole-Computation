...
feasibility of Misner's proposal.  We also test an exterior-supported
extension whose conserved cosmological tensor vanishes in the strong-field
region and reaches the constant positive-$\Lambda$ form outside a smooth
transition.  At fixed exterior $\Lambda$, this construction improves the raw,
...
Conservative refinement margins resolve the improvement, although it remains
modest at large $L$ and does not determine the global quasinormal spectrum.
...
pure-$\ell=2$ waveform.  The exterior-supported geometry uses a conserved
...
corresponding numerical comparison.  The fourth question requires a
...
Section~\ref{sec:exterior-supported} constructs and tests a fixed-angle-width
exterior-supported extension.
...
$104.2917$.  We report it as a global property rather than use it as a
small-shell acceptance criterion.  Local viability is tested through
curvature, the scalar potential, compactified coefficients, returned
radiation, and cost.
...
every $L$, matching the control ladder.  The bounded zeroth-order potential
is explicit in the IMEX split and transport remains implicit.  Because space
...
transition/cap pair.  The $L/M=640$ potential-representation error is
$3.47\times10^{-6}$ relative to the analytic transition minimum.  Every
background, spectral, and evolution-stability guard passes.
...
dense-versus-spectral comparison of the transition potential checks that the
transition's nonvanishing compact-coordinate width is resolved.  The preflight
...
\begin{equation}
 E_X(L;\mathcal J)
 =\frac{\|W_X-W_0\|_{\mathcal J}}{\|W_0\|_{\mathcal J}},
...
$W_X-W_0$ and normalized by $\|W_0\|_{\mathcal J}$.  If they decrease,
...
the two successive changes, and no Richardson order is assigned.  An
improvement is resolved when
...
nominal improvements remain resolved under the more pessimistic check that
adds this floor to the exterior upper bound and subtracts it from the uniform
lower bound.
...
\begin{figure*}[t]
 ...
 \caption{Raw fine-grid residuals relative to the same Schwarzschild waveform
 for uniform SdS (top) and the exterior-supported family (bottom).  Both
...
$\eta_\chi$ is the observed medium--fine change and the result is described
as grid-resolved, not continuum-converged.

\begin{table*}[t]
\caption{Raw QNM-window agreement with Schwarzschild.  Errors and numerical
 scales refer to $15\leq U/M\leq45$ and are percentages; $\eta$ is a
...
The fixed-background calculation supports the proposal in a limited sense:
at fixed exterior $\Lambda$, exterior support gives a resolved reduction of
the short ringing-waveform error.  The background audits exclude extra
horizons and an unresolved transition potential, and every evolution remains
stable through $100M$.  They do not exclude a later return or establish
long-time efficiency.  The transition mass in
Eq.~\eqref{eq:transition_mass_fixed_angle_family} is a global property of the
fixed-angle-width family and is not itself a falsifier.  These conclusions
concern only scalar waves on a fixed spherical background.
...
Sec.~\ref{sec:exterior-supported} leaves the strong-field metric exactly
Schwarzschild and trades distributed finite-$L$ bias for a distant conserved
anisotropic layer.  Fixing nonzero Chebyshev-angle widths for both the
...
to $8.6\%$ at $L/M=640$ and remains separated under conservative coupled
refinement margins.  This is a short-time waveform result, not a determination
of the global QNM spectrum or a long-time scattering result.
...
The uniform fixed-mode and localized-source production families use the
