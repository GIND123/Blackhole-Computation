...
feasibility of Misner's proposal.  We also formulate a further,
exterior-supported extension.  Its conserved cosmological tensor vanishes in
the strong-field region and reaches the constant positive-$\Lambda$ form
outside a smooth transition near the cosmological horizon.  We specify a
horizon-supported family and a three-way comparison with the same
Schwarzschild and uniform-SdS controls.  The production scalar-wave sequence
is under way, so no outcome of that comparison is included in the results
quoted here.  Application to the nonlinear Einstein equations motivates this
work but lies beyond its scope.
\end{abstract}
...
Four logically distinct questions should be separated.  First, can one
...
that recovery at the same exterior $\Lambda$?  Fourth, does finite $L$ actually
...
black-hole backgrounds using scalar waves.  It also constructs a conserved
exterior-supported geometry and defines the fixed-background test of the
third.  Section~\ref{sec:exterior-tests} gives the corresponding
horizon-supported production protocol.  The fourth question requires a
...
suggestion, we consider an exterior-supported cosmological tensor that leaves
the near zone exactly Schwarzschild and confines the modification to a smooth
outer layer.  This trade may permit smaller
...
Section~\ref{sec:exterior-supported} constructs an exterior-supported
extension and specifies the horizon-supported scalar-wave comparison.
...
dependence and specifies the separate large-$L$ calculation needed to resolve
a Price-law interval and subsequent cosmological decay in one waveform.
...
explicit, and Sec.~\ref{sec:exterior-tests} fixes the scalar-wave production
comparison now under way.  No outcome from that sequence is assumed below.
...
uniform SdS, $-r^2/L^2$ changes the metric and scalar potential even at the
photon sphere.  In the exterior-supported family, the metric, bridge boost,
and areal-radius potential are exactly Schwarzschild throughout the initial
data and strong-field region.  Any remaining waveform difference comes from
propagation through the outer layer, the cosmological boundary, and their
common clock construction.  The Schwarzschild, uniform-SdS, and
exterior-supported evolutions therefore separate strong-field bias from
outer-propagation bias at the same $L$.
...
The numerical family places the transition at a fixed fraction of the
cosmological-horizon scale.  Define
\begin{equation}
 D_L=1-\frac{2M}{r_c},
 \qquad
...
$\vartheta(r)=\arccos[2\rho_\chi(r)-1]$.  Chebyshev points are uniformly
spaced in $\vartheta$.  We fix
\begin{align}
 \beta&=\frac{9}{10},&
 R_1&=\beta r_c,&
 \vartheta_1&=\vartheta(R_1),\notag\\
 \vartheta_0&=2\vartheta_1,&
 \rho_{\chi,0}&=\frac{1+\cos\vartheta_0}{2},&
 R_0&=\frac{2M}{1-D_L\rho_{\chi,0}}.
 \label{eq:transition_radii}
\end{align}
Thus $2M<R_0<R_1<r_c$.  The standard compact $C^\infty$ step is
\begin{align}
 \eta(x)&=
 \begin{cases}
 0,&x\leq0,\\
...
and the exact-SdS cap have equal widths in $\vartheta$, which gives them
comparable Chebyshev point counts even when both occupy a narrow interval in
$\rho_\chi$.  In areal radius the layer is not thin.  Indeed,
\begin{equation}
 \frac{R_1}{r_c}=\beta,
 \qquad
 \frac{R_0}{r_c}\longrightarrow
 \frac{\beta}{4-3\beta}=\frac{9}{13}
 \quad(L/M\to\infty).
 \label{eq:transition_radii_limit}
\end{equation}
Both radii therefore move outward with $r_c=\mathcal O(L)$, while every fixed
strong-field radius eventually lies in the exact Schwarzschild region.  The
metric that preserves the exterior Kottler mass parameter $M$ is
\begin{equation}
...
\end{align}
The interpretation of this quantity depends on how the support moves with
$L$.  For the horizon-supported family in Eq.~\eqref{eq:transition_radii},
\begin{equation}
 \frac{\Delta m_{\rm tr}}{M}
 =\frac{\beta^3}{2}\left(\frac{r_c}{L}\right)^2\frac{r_c}{M}
 =\frac{\beta^3}{2}\left(\frac{r_c}{M}-2\right)
 \sim\frac{\beta^3}{2}\frac{L}{M}.
 \label{eq:transition_mass_horizon_family}
\end{equation}
The integrated Misner--Sharp mass therefore grows linearly rather than
remaining of order $M$.  This growth is the cosmological mass of an
$\mathcal O(L^3)$ horizon-scale volume with density $\mathcal O(L^{-2})$; it
does not represent a growing local disturbance in the strong-field region.
It is consequently reported as a global property, not used as a small-shell
acceptance criterion.  Local viability is tested through curvature, the
scalar potential, compactified coefficients, returned radiation, and cost.
...
feature.  Our layer instead has $\Delta=\mathcal O(L)$, so
$\lambda'=\mathcal O(L^{-3})$, $\lambda''=\mathcal O(L^{-4})$, and the source
eigenvalues and Ricci curvature in the layer are $\mathcal O(L^{-2})$.  The
construction replaces distributed strong-field bias by a low-curvature,
low-density modification in a distant, horizon-scale region.  The lapse
change there remains order unity, as it must near a cosmological horizon.  Its
compact-coordinate width still shrinks with $L$, which creates a distinct
resolution question.
...
\end{align}
Equation~\eqref{eq:transition_radii_limit} implies $R_0\to\infty$.  Hence
every fixed $r$ eventually lies below $R_0$, where $\chi_L=0$ and
\begin{equation}
...
Eq.~\eqref{eq:schwarz_coefficients}.
...
\begin{align}
 ...
 P_\chi&=\frac{D_L}{2M}
 \left[\ell(\ell+1)+rf_\chi'(r)\right].
...
\end{equation}
The clock limit must account for $R_0,R_1=\mathcal O(r_c)$.  Split the
integral at $R_0(L)$.  Below that radius the integrand is exactly the
Schwarzschild minimal-gauge integrand; its omitted tail is
$\mathcal O(M^2/R_0)$.  In the remaining integral set $r=xr_c$.  There
$1+B_\chi=8M^2r_c^{-2}(x^{-2}-\chi_L)$, while the common zero of numerator
and denominator at $x=1$ is removable.  The transition and outer-cap
integrand is $8M^2/r^2+\mathcal O(M^3/r_c^3)$ at fixed $x$.  Combining its
integral with the missing Schwarzschild tail above $R_0$, and using
$R_0/r_c\to9/13$, gives
\begin{equation}
 q_\chi=q_0-\frac{8M^2}{r_c}
 +\mathcal O\!\left(\frac{M^3}{r_c^2}\right)
 \longrightarrow q_0=4M\ln\frac{r_0}{r_0-2M}.
 \label{eq:exterior_clock_flat_limit}
\end{equation}
Thus this horizon-supported family shares the common Schwarzschild clock used
above.  The production audit records $q_\chi$ at every $L$ and compares it
with the asymptotic formula in Eq.~\eqref{eq:exterior_clock_flat_limit}.

\subsection{Production scalar-wave comparison}
\label{sec:exterior-tests}

The construction defines a fixed-background hypothesis: moving the
...
The production sequence uses the compact pure-$\ell=2$ datum and
$L/M=80,160,320,640$.  Each exterior-supported evolution runs to
$\tau=200M$ with RK222, $3/2$ dealiasing, and waveform output every $0.03M$.
The coarse, medium, and fine spatial resolutions are respectively
$(384,512,768)$, $(768,1024,1536)$, $(1024,1536,2048)$, and
$(1792,2048,2304)$ as $L/M$ increases.  The corresponding timesteps are
$0.005M$, $0.00375M$, and $0.0025M$ for every $L$, exactly the timestep
ladder of the frozen controls.  The bounded zeroth-order potential term is on
the explicit side of the IMEX split in the exterior-supported runs; transport
remains implicit.  This changes the linear solve, not the continuum system,
and its numerical effect must remain below the quoted discretization scale.
A representative matched-split or timestep-halving check is therefore
required before the waveform improvement is interpreted.
The chosen transition follows pilot profile comparisons and is held fixed
throughout this production sequence.

For the new geometry the scalar potential is
\begin{equation}
 V_\ell^\chi=f_\chi\left[
 \frac{\ell(\ell+1)}{r^2}+\frac{f_\chi'}{r}\right].
...
coefficients, and a nonnegative represented potential.  A
dense-versus-spectral comparison of the transition potential checks that its
shrinking compact-coordinate width is resolved.  The final background table
will give $r_c$, $R_0$, $R_1$, the compact layer width, transition and cap
point counts, $q_\chi$, coefficient extrema, potential resolution, and
$\Delta m_{\rm tr}/M$.  Runtime and step count are retained for the cost
comparison because higher spatial resolution may offset a gain in waveform
accuracy.  The evidence audit also compares the maximum stored reduction
constraint norms across the refinement ladder; it does not infer an
interval-resolved constraint history from endpoint snapshots.

Against the same separately evolved Schwarzschild waveform $W_0$, define
\begin{equation}
 E_\chi(L;\mathcal J)
 =\frac{\|W_\chi-W_0\|_{\mathcal J}}{\|W_0\|_{\mathcal J}}.
 \label{eq:exterior_waveform_error}
\end{equation}
The headline interval is the raw, unshifted outer-boundary waveform on
$0\leq U/M\leq80$.  No time translation, amplitude rescaling, time dilation,
or background transfer correction is fitted.  Cumulative windows
$0$--$40M$, $0$--$80M$, and $0$--$160M$, together with disjoint windows
$40$--$80M$ and $80$--$160M$, separate prompt and later propagation.  The
run to $200M$ exposes later changes hidden by the headline window.  Interior
observer signals are retained for a reflection check.  For each $L$, the
analysis must compare the record with the predicted tortoise-coordinate
round-trip time; absence of a return is informative only when that time lies
inside the evolved interval.  An apparent improvement is accepted only if
$E_\chi<E_{\rm SdS}$ at the same $L$ and the gap exceeds the combined
deterministic numerical scales of the candidate and uniform-SdS control.  The
common Schwarzschild floor is included separately when testing an absolute
$1\%$, $2\%$, or $5\%$ threshold.  Candidate convergence is measured on its
own unequal-resolution ladder; it is not paired grid by grid with the
archived control ladder.  Later windows are reported as diagnostic whenever
...
repeat the localized-source calculation.  Prompt time-domain ringing is a
waveform diagnostic, not a computation of the global quasinormal spectrum.
...
both boundaries.  We therefore make no QNM claim for the exterior-supported
geometry.

\subsection{Scope and acceptance criteria}

The fixed-background proposal is supported if the production sequence shows
a resolved error reduction relative to uniform SdS at the same $L$ and
exterior $\Lambda$, while avoiding extra horizons, a dominant secondary
potential barrier, significant returned radiation over the useful interval,
large local curvature or stiffness, and prohibitive cost.  The extensive
transition mass in Eq.~\eqref{eq:transition_mass_horizon_family} is an
expected property of this horizon-scale family and is not itself a
falsifier.  Failure to obtain a reduction beyond numerical sensitivity, or a
gain erased by transition scattering or computational cost, would disfavor
this particular construction.  These criteria concern only scalar waves on
a fixed spherical background.
...
The exterior-supported construction in
Sec.~\ref{sec:exterior-supported} defines the fixed-background benchmark now
under numerical test.  Its horizon-supported family leaves the strong-field
metric exactly Schwarzschild and is designed to reduce waveform bias at the
same exterior $\Lambda$.  It trades distributed finite-$L$ bias for a distant
anisotropic layer, compact-coordinate resolution cost, and possible
scattering.  The production sequence measures these effects; quantitative
interpretation awaits the completed convergence and late-window audit.  A
nonlinear implementation also requires a covariant, conserved completion;
prescribing a coordinate-dependent scalar $\Lambda(r)$ is inconsistent with
the Bianchi identity.

The ongoing sequence implements the comparison in
Sec.~\ref{sec:exterior-tests}, using the same scalar data, waveform clock,
norms, Schwarzschild reference, and uniform-SdS control.  Until every
resolution and late-window check is complete, it supports no claim that
exterior support improves Schwarzschild recovery.  Beyond that benchmark,
...
\subsection{\texorpdfstring{Large-$L$}{Large-L} crossover protocol}

A direct Price-to-cosmological crossover requires a larger cosmological scale
than the sequence above.  The headline observable is fixed at $r=8M$ rather
than moved between finite radius and the outer boundary.  This choice tests
the timelike dipole Price index $p=5$ and keeps the expected cosmological
entry at a computationally accessible time.  The first production target is
$L/M=640$, evolved with the same compact dipole velocity datum and a matched
Schwarzschild reference through at least $\kappa_cU=4$.

The Price regime must remain within $5\%$ of $p=5$ for a continuous interval
of at least $150M$ containing $U=300M$.  Cosmological entry requires
$\gamma_{\rm eff}/\kappa_c$ to remain within $10\%$ of unity for
$\Delta(\kappa_cU)\geq0.4$.  Both intervals must lie at least a factor of ten
above the time-dependent numerical floor.  Three spatial resolutions, a
halved-timestep run, and a smoothing-window sweep must give the same ordered
Price interval, departure, and cosmological entry within their quoted
sensitivities.  Outer-boundary and other finite-radius signals are secondary
diagnostics; regimes from different observers are never combined into a
single crossover.  If no waveform satisfies both criteria, the calculation
is reported as a resolved null result rather than as a transition.  A
screening archive is not publication evidence, so only a clean frozen ladder
can support the paper's final tail figure and transition time.
